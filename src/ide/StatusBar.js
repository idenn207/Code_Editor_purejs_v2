...
(function(CodeEditor) {
  ...
  class StatusBar {
    ...
    addItem(id, options) {
      var self = this;
      options = options || {};
      var position = options.position || StatusBar.POSITIONS.LEFT;
    ...
    updateItem(id, updates) {
      ...
      if (updates.text !== undefined) {
        var textEl = element.querySelector('.ide-status-text');
        if (textEl) {
      ...
      if (updates.icon !== undefined) {
        var iconEl = element.querySelector('.ide-status-icon');
        if (updates.icon) {
          if (!iconEl) {
    ...
    setProblems(errors, warnings) {
      var text = errors > 0 || warnings > 0 ? errors + ' ' + warnings : 'No Problems';
      this.updateItem('problems', { text: text });
    ...
    setMessage(message, timeout) {
      ...
      if (timeout > 0) {
        setTimeout(function() {
          self.updateItem('message', { visible: false });
        }, timeout);
      }
    }
    ...
    _insertItem(element, position, priority) {
      var container = position === 'left' ? this._leftElement : this._rightElement;
      ...
      for (var i = 0; i < items.length; i++) {
        var item = items[i];
        var itemPriority = parseInt(item.dataset.priority || '0', 10);
    ...
    _bindEditorEvents() {
      ...
      // Language change
      if (this._editor.on) {
        this._editor.on('languageChange', function(data) {
    ...
    _updateSelectionInfo() {
      ...
      var hasSelection = selection.start !== selection.end;
  ...
  CodeEditor.IDE.StatusBar = StatusBar;

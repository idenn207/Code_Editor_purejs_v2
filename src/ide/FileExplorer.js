...
export class FileExplorer {
  ...
  selectByPath(path) {
    ...
    const node = this._rootNode.findByPath(path);
    if (node) {
  ...
  _bindEvents() {
    // Click on tree item
    this._treeElement.addEventListener('click', (e) => this._handleClick(e));

    // Double click to open file
    this._treeElement.addEventListener('dblclick', (e) => this._handleDoubleClick(e));

    // Keyboard navigation
    this._treeElement.addEventListener('keydown', (e) => this._handleKeyDown(e));
  ...
  _render() {
    ...
    const visibleNodes = this._rootNode.getVisibleNodes();

    for (const node of visibleNodes) {
      const item = this._renderNode(node);
  ...
  _selectNode(node) {
    ...
    // Update DOM
    this._treeElement.querySelectorAll('.ide-tree-item').forEach((el) => {
      el.classList.toggle('selected', el.dataset.id === node.id);
    });

    this._emit('selectionChange', { node });
  }
   ...
   */
  _scrollToNode(node) {
    const item = this._treeElement.querySelector(`[data-id="${node.id}"]`);
    if (item) {
  ...
  _handleClick(e) {
    ...
    if (node.type === 'file') {
      this._emit('fileSelect', { node, path: node.path });
    }
  }
  ...
  _handleDoubleClick(e) {
    ...
    if (node.type === 'directory') {
      ...
      this._emit('fileOpen', { node, path: node.path });
    }
  }
  ...
  _handleKeyDown(e) {
    ...
    const visibleNodes = this._rootNode.getVisibleNodes();
    const currentIndex = visibleNodes.indexOf(this._selectedNode);
    ...
    switch (e.key) {
      ...
      case 'ArrowRight':
        ...
        if (this._selectedNode?.type === 'directory') {
          if (!this._selectedNode.expanded) {
            ...
            // Move to first child
            const firstChild = this._selectedNode.children[0];
      ...
      case 'ArrowLeft':
        e.preventDefault();
        if (this._selectedNode?.type === 'directory' && this._selectedNode.expanded) {

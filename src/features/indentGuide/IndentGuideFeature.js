 ...
 *
 * @example
 * const indentGuide = new IndentGuideFeature(editor, { tabSize: 2 });
 */
export class IndentGuideFeature {
  // ----------------------------------------
  // Instance Properties
  // ----------------------------------------
  _editor = null;
  ...
  _bindEvents() {
    this._boundHandleChange = () => this._scheduleUpdate();
  ...
  _updateGuides() {
    ...
    for (let lineIndex = 0; lineIndex < lineCount; lineIndex++) {
      ...
      const indentLevel = this._getIndentLevel(lineText);

      // Render a guide for each indent level on this line
      for (let level = 1; level <= indentLevel; level++) {
        const column = (level - 1) * this._tabSize;
        const colorClass = `ec-indent-guide-${((level - 1) % COLOR_COUNT) + 1}`;

...
(function(CodeEditor) {
  ...
  class SelectionCollection {
    ...
    adjustForDocumentChange(changeStart, deletedLength, insertedLength) {
      var delta = insertedLength - deletedLength;
      var changeEnd = changeStart + deletedLength;

      for (var i = 0; i < this._selections.length; i++) {
        var sel = this._selections[i];
        var newStart = sel._startOffset;
        var newEnd = sel._endOffset;
     ...
     */
    clone() {
      var clone = new SelectionCollection([]);
    ...
    hasSelectionAt(start, end) {
      var normStart = Math.min(start, end);
      var normEnd = Math.max(start, end);
    ...
    sorted(descending = false) {
      var sorted = [...this._selections].sort((a, b) => a.start - b.start);
    ...
    _normalize() {
      ...
      var primarySel = this._selections[this._primaryIndex];
      var primaryStart = primarySel.start;
      var primaryEnd = primarySel.end;

      // Sort by start offset
      this._selections.sort((a, b) => a.start - b.start);

      // Merge overlapping/adjacent selections
      var merged = [this._selections[0]];
      for (var i = 1; i < this._selections.length; i++) {
        var current = this._selections[i];
      ...
      this._primaryIndex = 0;
      for (var j = 0; j < this._selections.length; j++) {
        var sel = this._selections[j];
        if (sel.start <= primaryStart && sel.end >= primaryEnd) {
          this._primaryIndex = j;
          break;
        }
  ...
  CodeEditor.SelectionCollection = SelectionCollection;

...
(function(CodeEditor) {
  ...
  class FileNode {
     ...
     */
    constructor(name, path, type, options) {
      type = type || 'file';
      options = options || {};
    ...
    findByPath(targetPath) {
      ...
      if (this._type === 'directory' && this._children) {
        for (var i = 0; i < this._children.length; i++) {
          var found = this._children[i].findByPath(targetPath);
          if (found) {
            return found;
          }
        }
    ...
    findById(id) {
      ...
      if (this._type === 'directory' && this._children) {
        for (var i = 0; i < this._children.length; i++) {
          var found = this._children[i].findById(id);
          if (found) {
            return found;
          }
        }
    ...
    _collectVisibleNodes(nodes) {
      ...
      if (this._type === 'directory' && this._expanded && this._children) {
        for (var i = 0; i < this._children.length; i++) {
          this._children[i]._collectVisibleNodes(nodes);
        }
      }
    }

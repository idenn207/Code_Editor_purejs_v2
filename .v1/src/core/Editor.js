...
import { AutoClosingPairs } from '../features/AutoClosingPairs.js';
import { CommentToggle } from '../features/CommentToggle.js';
import { HoverTooltip } from '../features/HoverTooltip.js';
import { LineOperations } from '../features/LineOperations.js';
...
const DEFAULT_OPTIONS = {
  ...
  autoClosingPairs: true, // Enable auto-closing pairs by default
  commentToggle: true, // Enable comment toggling by default
  lineOperations: true, // Enable line operations by default
};
...
export class Editor {
  ...
  _autoComplete = null;
  _hoverTooltip = null;
  _autoClosingPairs = null;
  _commentToggle = null;
  _lineOperations = null;
  ...
  _initialize() {
    ...
    // Create auto-closing pairs (if enabled)
    if (this._options.autoClosingPairs) {
      this._autoClosingPairs = new AutoClosingPairs(this, {
        language: this._options.language,
      });
    }

    // Create comment toggle (if enabled)
    if (this._options.commentToggle) {
      this._commentToggle = new CommentToggle(this, {
        language: this._options.language,
      });
    }

    // Create line operations (if enabled)
    if (this._options.lineOperations) {
      this._lineOperations = new LineOperations(this);
    }
  ...
  dispose() {
    ...
    this._autoClosingPairs?.dispose();
    this._commentToggle?.dispose();
    this._lineOperations?.dispose();
